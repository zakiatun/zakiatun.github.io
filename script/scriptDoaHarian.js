...
function itemPlaylist(index, judul, pdf, latex){
    ...
    favoritTrack.innerHTML = 'Favoritkan';
    document.querySelector('#more-' + index).appendChild(favoritTrack); 
...
for(let i = 0; i < objHarian.length; i++){
   itemPlaylist(i, objHarian[i].judul, objHarian[i].pdf, objHarian[i].latex);
}
...
for(let i = 0; i < lihatPdf.length; i++){
    lihatPdf[i].addEventListener('click', function(){
        window.open('../pdf/'+objHarian[i].pdf);
    });
}
...
for(let i = 0; i < lihatLatex.length; i++){
    lihatLatex[i].addEventListener('click', function(){
        window.open('../latex/'+objHarian[i].latex);
    });
}
